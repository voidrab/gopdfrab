...
func DecodeImageRGBA(dict PDFDict, resources PDFDict) (*image.RGBA, error) {
	...
	filters := filterNames(dict.Entries["Filter"])
	last := ""
	if len(filters) > 0 {
...
func decodeImageRawSamples(dict PDFDict) ([]byte, error) {
	filters := filterNames(dict.Entries["Filter"])
	hasLZW := false
	...
	var err error
	if hasLZW {
		data, err = decodeLZW(dict.RawStream)
	} else {
	...
	columns := dictInt(parms, "Columns", dictInt(dict, "Width", 1))
	colors := dictInt(parms, "Colors", 1)
	bpc := dictInt(parms, "BitsPerComponent", dictInt(dict, "BitsPerComponent", 8))
	if predictor == 2 {
...
func unpackSamplesToRGBA(dict PDFDict, resources PDFDict, data []byte, width, height int) (*image.RGBA, error) {
	bpc := dictInt(dict, "BitsPerComponent", 8)
	cs := resolveImageColorSpace(dict, resources)
	ncomp := colorSpaceComponents(cs)
	if dict.Entries["ImageMask"] == PDFBoolean(true) {
		ncomp = 1
		bpc = 1
	}
	...
	for y := 0; y < height; y++ {
		...
		for x := 0; x < width; x++ {
			for c := 0; c < ncomp; c++ {
				...
				raw := float64(readBits(data, bitOffset, bpc))
			...
			if dict.Entries["ImageMask"] == PDFBoolean(true) {
				...
				r, g, b = ResolveColor(cs, comps, resources)
			}
...
func resolveImageColorSpace(dict PDFDict, resources PDFDict) PDFValue {
	if dict.Entries["ImageMask"] == PDFBoolean(true) {
		return PDFName{Value: "DeviceGray"}
	}
	cs := dict.Entries["ColorSpace"]
	if name, ok := cs.(PDFName); ok {
		if named, ok := lookupNamedColorSpace(name.Value, resources); ok {
			return named
		}
	}
	return cs
...
func imageDecodeArray(dict PDFDict, cs PDFValue, ncomp, bpc int) []float64 {
	if arr, err := floatArray(dict.Entries["Decode"]); err == nil && len(arr) == 2*ncomp {
		return arr
	}
...
func indexedHead(cs PDFValue) (PDFArray, bool) {
	arr, ok := cs.(PDFArray)
	if !ok || len(arr) == 0 {
		return nil, false
	}
...
func decodeJPEGImage(dict PDFDict) (*image.RGBA, error) {
