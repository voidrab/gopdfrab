...
func TestProfile_NewProfileIsEmpty(t *testing.T) {
	...
	if p.Has(check.Checks.Transparency.ImageWithSoftMask) {
...
func TestProfile_CopyOnWrite(t *testing.T) {
	...
	_ = PDFA_1B.RemoveCheck(check.Checks.Transparency.ImageWithSoftMask)
	if !PDFA_1B.Has(check.Checks.Transparency.ImageWithSoftMask) {
		t.Error("RemoveCheck() modified PDFA_1B")
	}

	empty := NewProfile(A_1B)
	_ = empty.AddCheck(check.Checks.Structure.FileHeaderSignature)
	if empty.Has(check.Checks.Structure.FileHeaderSignature) {
...
func TestProfile_AddCheck(t *testing.T) {
	p := PDFA_1B.Clear().
		AddCheck(check.Checks.Transparency.ImageWithSoftMask, check.Checks.Structure.ObjectFraming)
	if len(p.Checks()) != 2 {
	...
	if !p.Has(check.Checks.Structure.ObjectFraming) {
		t.Error("added ObjectFraming not found")
	}
	if p.Has(check.Checks.Metadata.PDFAIdentifierMissing) {
		t.Error("non-added check should not be present")
	}
}

func TestProfile_RemoveCheck(t *testing.T) {
	p := PDFA_1B.RemoveCheck(check.Checks.Transparency.ImageWithSoftMask)
	if p.Has(check.Checks.Transparency.ImageWithSoftMask) {
		t.Error("removed check still present")
	}
	if !p.Has(check.Checks.Structure.FileHeaderSignature) {
		t.Error("unrelated check should still be present after RemoveCheck")
	}
	empty := PDFA_1B.Clear()
	_ = empty.RemoveCheck(check.Checks.Transparency.ImageWithSoftMask)
}
...
func TestProfile_ChecksOrder(t *testing.T) {
	...
	if len(got) != len(all) {
		t.Fatalf("check.Checks() length mismatch: %d vs %d", len(got), len(all))
	}
	for i := range all {
		if got[i].ID() != all[i].ID() {
			t.Errorf("check.Checks()[%d].id = %d, want %d (%s)", i, got[i].ID(), all[i].ID(), all[i].Name())
		}
	}
}
...
func TestProfile_AllowsCatalogPairs(t *testing.T) {
	chk := check.Checks.Transparency.ImageWithSoftMask
...
func header61checks() []check.Check {
	s := check.Checks.Structure
	return []check.Check{
...
func TestVerifyProfile_FullProfileMatchesVerify(t *testing.T) {
	...
	resVerify, err := Verify(doc, A_1B)
	if err != nil {
		t.Fatalf("Verify: %v", err)
	}
	resProfile, err := VerifyProfile(doc, PDFA_1B)
	if err != nil {
...
func TestVerifyProfile_RemoveCheckSuppressesViolation(t *testing.T) {
	...
	full, _ := VerifyProfile(doc, PDFA_1B)
	if full.Valid {
...
func TestVerifyProfile_UndefinedLevelReturnsError(t *testing.T) {
	...
	_, err = VerifyProfile(doc, NewProfile(Undefined))
	if err == nil {
