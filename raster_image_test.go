...
func TestDecodeImageRGBA8BitGray(t *testing.T) {
	dict := PDFDict{
		Entries: map[string]PDFValue{
			"Width": PDFInteger(2), "Height": PDFInteger(1),
	...
	img, err := DecodeImageRGBA(dict, PDFDict{})
	if err != nil {
...
func TestDecodeImageRGBA1BitMonochrome(t *testing.T) {
	...
	dict := PDFDict{
		Entries: map[string]PDFValue{
			"Width": PDFInteger(8), "Height": PDFInteger(1),
...
func TestDecodeImageRGBA2And4Bit(t *testing.T) {
	...
	img, err := DecodeImageRGBA(dict4, PDFDict{})
	if err != nil {
		t.Fatalf("DecodeImageRGBA: %v", err)
	}
	...
	dict2 := PDFDict{
		Entries: map[string]PDFValue{
			"Width": PDFInteger(4), "Height": PDFInteger(1),
			"BitsPerComponent": PDFInteger(2),
			"ColorSpace":       PDFName{Value: "DeviceGray"},
		},
		HasStream: true,
		RawStream: []byte{0b11100100},
	}
	img2, err := DecodeImageRGBA(dict2, PDFDict{})
	if err != nil {
		t.Fatalf("DecodeImageRGBA: %v", err)
	}
...
func TestDecodeImageRGBADecodeArrayInversion(t *testing.T) {
	dict := PDFDict{
		Entries: map[string]PDFValue{
			...
			"BitsPerComponent": PDFInteger(8),
			"ColorSpace":       PDFName{Value: "DeviceGray"},
			"Decode":           PDFArray{PDFInteger(1), PDFInteger(0)},
		},
		HasStream: true,
		RawStream: []byte{0x00, 0xFF},
	}
...
func TestDecodeImageRGBAJPEGRoundTrip(t *testing.T) {
	...
	dict := PDFDict{
		Entries: map[string]PDFValue{
			"Width": PDFInteger(4), "Height": PDFInteger(4),
			"BitsPerComponent": PDFInteger(8),
			"ColorSpace":       PDFName{Value: "DeviceRGB"},
			"Filter":           PDFName{Value: "DCTDecode"},
		},
		HasStream: true,
		RawStream: buf.Bytes(),
	}
...
func TestDecodeImageRGBAUnsupportedCodecPlaceholder(t *testing.T) {
	dict := PDFDict{
		Entries: map[string]PDFValue{
			"Width": PDFInteger(3), "Height": PDFInteger(3),
			"BitsPerComponent": PDFInteger(1),
			"ColorSpace":       PDFName{Value: "DeviceGray"},
			"Filter":           PDFName{Value: "CCITTFaxDecode"},
		},
		HasStream: true,
		RawStream: []byte{0x00},
	}
