...
func TestFontDictFixerAddsIdentityCIDToGIDMap(t *testing.T) {
	cidFont := NewPDFDict()
	cidFont.Entries["Type"] = PDFName{Value: "Font"}
	cidFont.Entries["Subtype"] = PDFName{Value: "CIDFontType2"}
	cidFont.Entries["BaseFont"] = PDFName{Value: "Test"}

	type0 := NewPDFDict()
	type0.Entries["Type"] = PDFName{Value: "Font"}
	type0.Entries["Subtype"] = PDFName{Value: "Type0"}
	type0.Entries["DescendantFonts"] = PDFArray{cidFont}

	trailer := NewPDFDict()
	trailer.Entries["Root"] = PDFDict{Entries: map[string]PDFValue{"Font": type0}}
	...
	got, ok := cidFont.Entries["CIDToGIDMap"].(PDFName)
	if !ok || got.Value != "Identity" {
		t.Fatalf("CIDToGIDMap = %#v, want PDFName{Identity}", cidFont.Entries["CIDToGIDMap"])
	}
...
func TestFontDictFixerAppliesOnlyToCIDToGIDMapMissing(t *testing.T) {
	...
	for _, c := range AllChecks() {
		want := c == Checks.Font.CIDToGIDMapMissing
