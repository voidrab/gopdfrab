...
func pageContentBytes(page PDFDict) ([]byte, error) {
	var out []byte
	switch c := page.Entries["Contents"].(type) {
	case PDFDict:
		data, err := decodeStream(c)
		if err != nil {
			return nil, err
		}
		out = data
...
func (r *renderer) execContent(data []byte, resources PDFDict, gs renderState) {
	...
	newContentScanner(data).scan(func(op string, operands []PDFValue) {
		...
		case "g":
			...
			gs.fillCS = PDFName{Value: "DeviceGray"}
		case "G":
			a := nums(1)
			gs.strokeRGB = [3]float64{a[0], a[0], a[0]}
			gs.strokeCS = PDFName{Value: "DeviceGray"}
		case "rg":
			a := nums(3)
			gs.fillRGB = [3]float64{a[0], a[1], a[2]}
			gs.fillCS = PDFName{Value: "DeviceRGB"}
		case "RG":
			a := nums(3)
			gs.strokeRGB = [3]float64{a[0], a[1], a[2]}
			gs.strokeCS = PDFName{Value: "DeviceRGB"}
		case "k":
			a := nums(4)
			gs.fillRGB[0], gs.fillRGB[1], gs.fillRGB[2] = cmykToRGB(a)
			gs.fillCS = PDFName{Value: "DeviceCMYK"}
		case "K":
			a := nums(4)
			gs.strokeRGB[0], gs.strokeRGB[1], gs.strokeRGB[2] = cmykToRGB(a)
			gs.strokeCS = PDFName{Value: "DeviceCMYK"}
		case "cs":
		...
		case "sc", "scn":
			if comps := numericOperands(operands); len(comps) > 0 && gs.fillCS != nil {
				r, g, b := ResolveColor(gs.fillCS, comps, resources)
				gs.fillRGB = [3]float64{r, g, b}
			}
		case "SC", "SCN":
			if comps := numericOperands(operands); len(comps) > 0 && gs.strokeCS != nil {
				r, g, b := ResolveColor(gs.strokeCS, comps, resources)
		...
		case "T*":
			...
			gs.tm = gs.tlm
		case "Tj":
			r.showText(shownStringBytes(op, operands), resources, &gs)
		case "'":
			gs.tlm = Matrix{A: 1, D: 1, F: -gs.leading}.Mul(gs.tlm)
			gs.tm = gs.tlm
			r.showText(shownStringBytes(op, operands), resources, &gs)
...
func numericOperands(operands []PDFValue) []float64 {
	var out []float64
	for _, v := range operands {
		if f, ok := pdfNumberToFloat(v); ok {
			out = append(out, f)
		}
	}
	return out
...
func resolveOperandColorSpace(operands []PDFValue, resources PDFDict) PDFValue {
	if len(operands) == 0 {
		return nil
	}
	...
	if cs, ok := lookupNamedColorSpace(name.Value, resources); ok {
		return cs
	}
	return name
}
...
func (r *renderer) applyExtGState(operands []PDFValue, resources PDFDict, gs *renderState) {
	if len(operands) == 0 {
		return
	}
	name, ok := operands[len(operands)-1].(PDFName)
	...
	extGStates, _ := resources.Entries["ExtGState"].(PDFDict)
	egs, ok := extGStates.Entries[name.Value].(PDFDict)
	if !ok {
		return
	}
	if ca, ok := pdfNumberToFloat(egs.Entries["ca"]); ok {
		gs.fillAlpha = ca
	}
	if CA, ok := pdfNumberToFloat(egs.Entries["CA"]); ok {
		gs.strokeAlpha = CA
...
func (r *renderer) doXObject(operands []PDFValue, resources PDFDict, gs *renderState) {
	if len(operands) == 0 {
		return
	}
	name, ok := operands[len(operands)-1].(PDFName)
	...
	case "Form":
		...
		childGS := *gs
		if m, err := floatArray(xobj.Entries["Matrix"]); err == nil && len(m) == 6 {
...
func (r *renderer) paintImage(xobj PDFDict, resources PDFDict, gs *renderState) {
	...
	var smW, smH int
	if sm, ok := xobj.Entries["SMask"].(PDFDict); ok {
	...
	for y := y0; y < y1; y++ {
		for x := x0; x < x1; x++ {
			...
			col := clampInt(int(p.X*float64(w)), 0, w-1)
			row := clampInt(int((1-p.Y)*float64(h)), 0, h-1)
			px := img.RGBAAt(col, row)
			alpha := float64(px.A) / 255 * gs.fillAlpha
			if smask != nil {
				scol := clampInt(col*smW/w, 0, smW-1)
				srow := clampInt(row*smH/h, 0, smH-1)
...
func (r *renderer) applyTf(operands []PDFValue, resources PDFDict, gs *renderState) {
	if len(operands) < 2 {
		return
	}
	name, ok := operands[len(operands)-2].(PDFName)
	if !ok {
		return
	}
	size, _ := pdfNumberToFloat(operands[len(operands)-1])
	fonts, _ := resources.Entries["Font"].(PDFDict)
	fd, ok := fonts.Entries[name.Value].(PDFDict)
	if !ok {
		return
	}
	gs.font = fd
...
func (r *renderer) showTextArray(operands []PDFValue, resources PDFDict, gs *renderState) {
	if len(operands) == 0 {
		return
	}
	arr, ok := operands[len(operands)-1].(PDFArray)
	if !ok {
		return
	}
	for _, item := range arr {
		switch v := item.(type) {
		case PDFString:
			r.showText(decodePDFLiteralStringBytes(v.Value), resources, gs)
		case PDFHexString:
			r.showText(decodePDFHexStringBytes(v.Value), resources, gs)
		default:
			if adj, ok := pdfNumberToFloat(v); ok {
...
func (r *renderer) showText(raw []byte, resources PDFDict, gs *renderState) {
...
func (r *renderer) fontInfoFor(font PDFDict, resources PDFDict) *fontInfo {
	key := pdfValuePointer(font.Entries)
...
func buildFontInfo(font PDFDict, resources PDFDict) *fontInfo {
	if df, ok := font.Entries["DescendantFonts"].(PDFArray); ok && len(df) > 0 {
...
func buildCompositeFontInfo(desc PDFDict) *fontInfo {
	fi := &fontInfo{bytesPerCode: 2, defaultWidth: 1000, widths: map[int]float64{}}
	if dw, ok := pdfNumberToFloat(desc.Entries["DW"]); ok {
		fi.defaultWidth = dw
	}
	if w, ok := desc.Entries["W"].(PDFArray); ok {
	...
	if c2g, ok := desc.Entries["CIDToGIDMap"].(PDFDict); ok && c2g.HasStream {
		if data, err := decodeStream(c2g); err == nil {
	...
	desc2, _ := desc.Entries["FontDescriptor"].(PDFDict)
	if ff2, ok := desc2.Entries["FontFile2"].(PDFDict); ok {
		data, err := decodeStream(ff2)
		if err == nil {
			if tables, ok := parseSfnt(data); ok {
	...
	if ff3, ok := desc2.Entries["FontFile3"].(PDFDict); ok {
		data, err := decodeStream(ff3)
		if err == nil {
...
func buildSimpleFontInfo(font PDFDict) *fontInfo {
	fi := &fontInfo{bytesPerCode: 1, widths: map[int]float64{}}
	firstChar := dictInt(font, "FirstChar", 0)
	if widths, ok := font.Entries["Widths"].(PDFArray); ok {
		for i, w := range widths {
			if v, ok := pdfNumberToFloat(w); ok {
				fi.widths[firstChar+i] = v
			}
		}
	}

	desc, _ := font.Entries["FontDescriptor"].(PDFDict)
	if mw, ok := pdfNumberToFloat(desc.Entries["MissingWidth"]); ok {
		fi.defaultWidth = mw
	}
	...
	if ff2, ok := desc.Entries["FontFile2"].(PDFDict); ok {
		data, err := decodeStream(ff2)
		if err == nil {
			if tables, ok := parseSfnt(data); ok {
				fi.glyphFor = func(code int) (GlyphPath, bool) {
	...
	if ff3, ok := desc.Entries["FontFile3"].(PDFDict); ok {
		data, err := decodeStream(ff3)
		if err == nil {
	...
	if gidMap != nil && code >= 0 && code < 256 {
		if name := names[code]; name != "" {
			if uni, ok := glyphNameToWinAnsiCode(name); ok {
				if gid, ok := gidMap[winAnsiToUnicode[uni]]; ok {
					return int(gid)
				}
			}
		}
...
// glyphNameToWinAnsiCode reverse-looks-up winAnsiGlyphName for name,
...
func glyphNameToWinAnsiCode(name string) (int, bool) {
	for i, n := range winAnsiGlyphName {
		if n == name {
			return i, true
		}
	}
...
func resolveSimpleEncoding(enc PDFValue) [256]string {
	var names [256]string
	switch e := enc.(type) {
	case PDFName:
		switch e.Value {
		case "WinAnsiEncoding":
			names = winAnsiGlyphName
		default:
			names = standardEncoding
		}
	case PDFDict:
