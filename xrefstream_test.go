...
func TestXRefStreamWithObjStm(t *testing.T) {
	...
	if ref, ok := page.Entries["_ref"].(PDFRef); !ok || ref.ObjNum != 3 {
...
func assertOnePageGraph(t *testing.T, graph PDFValue) PDFDict {
	t.Helper()
	root, ok := graph.(PDFDict).Entries["Root"].(PDFDict)
	if !ok {
		t.Fatalf("Root did not resolve to a dict")
	}
	if !EqualPDFValue(root.Entries["Type"], PDFName{Value: "Catalog"}) {
	...
	kids, ok := pages.Entries["Kids"].(PDFArray)
	if !ok || len(kids) != 1 {
		t.Fatalf("Pages/Kids = %v, want a 1-element array", pages.Entries["Kids"])
	}
	page, ok := kids[0].(PDFDict)
	if !ok {
		t.Fatalf("Kids[0] did not resolve to a dict")
	}
	if !EqualPDFValue(page.Entries["Type"], PDFName{Value: "Page"}) {
...
func assertContentStream(t *testing.T, doc *Document, page PDFDict, want string) {
	t.Helper()
	contents, ok := page.Entries["Contents"].(PDFDict)
