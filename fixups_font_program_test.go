...
func fixtureTrailer(t *testing.T, path string) (trailer PDFDict, closeDoc func()) {
	t.Helper()
...
func assertCheckClearedByWrite(t *testing.T, trailer PDFDict, c Check) {
	t.Helper()
	var buf bytes.Buffer
	if err := WriteDocument(&buf, trailer); err != nil {
		t.Fatalf("WriteDocument: %v", err)
	}
	doc, err := Open(writeTempPDF(t, "font_program_fixed.pdf", buf.Bytes()))
	if err != nil {
		t.Fatalf("Open(written output): %v", err)
	}
	defer doc.Close()
	res, err := doc.Verify(A_1B)
	if err != nil {
...
func findFontBySubtype(t *testing.T, trailer PDFDict, subtype string) PDFDict {
	t.Helper()
	var found PDFDict
	ok := false
	walkDicts(trailer, map[uintptr]bool{}, func(d PDFDict) {
		if ok || (d.Entries["Type"] != PDFName{Value: "Font"}) {
			return
		}
		st, _ := d.Entries["Subtype"].(PDFName)
		if st.Value == subtype {
			found, ok = d, true
		}
	})
...
func TestFontMetricFixerAppliesOnlyToAdvanceWidthMismatch(t *testing.T) {
	...
	for _, c := range AllChecks() {
		want := c == Checks.Font.AdvanceWidthMismatch
...
func TestFontSubsetMetaFixerAppliesOnlyToCharSetAndCIDSet(t *testing.T) {
	...
	for _, c := range AllChecks() {
		want := c == Checks.Font.Type1SubsetCharSet || c == Checks.Font.CIDSubsetCIDSet
		if got := fixer.Applies(c); got != want {
			t.Errorf("Applies(%s/%d) = %v, want %v", c.Clause(), c.Subclause(), got, want)
		}
	}
...
func runFixerAndCheckIdempotent(t *testing.T, fixer Fixer, trailer *PDFDict) {
	t.Helper()
...
func TestFontMetricFixerCorrectsSimpleTrueTypeWidths(t *testing.T) {
	...
	assertCheckClearedByWrite(t, trailer, Checks.Font.AdvanceWidthMismatch)
}
...
func TestFontSubsetMetaFixerSynthesizesType1CharSet(t *testing.T) {
	...
	cs, ok := desc.Entries["CharSet"].(PDFString)
	if !ok || cs.Value == "" {
		t.Fatalf("CharSet = %v, want a non-empty PDFString", desc.Entries["CharSet"])
	}
	assertCheckClearedByWrite(t, trailer, Checks.Font.Type1SubsetCharSet)
}
...
func TestFontSubsetMetaFixerSynthesizesCFFCIDSet(t *testing.T) {
	for _, path := range []string{
		...
		t.Run(path, func(t *testing.T) {
			...
			if dirty, _ := cidSet.Entries["_dirty"].(PDFBoolean); !bool(dirty) {
				t.Errorf("CIDSet stream not marked dirty")
			}
			assertCheckClearedByWrite(t, trailer, Checks.Font.CIDSubsetCIDSet)
		})
	}
}
