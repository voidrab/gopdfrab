...
// trailing inlineImageRaw operand's bytes verbatim, ignoring the parsed
// params -- see inlineImageRaw's doc comment in content.go.
...
// inlineImageBytes extracts the trailing inlineImageRaw operand
// scanInlineImage appends to an "INLINEIMAGE" op's operands.
func inlineImageBytes(operands []PDFValue) ([]byte, bool) {
	if len(operands) == 0 {
		return nil, false
	}
...
// untouched image is passed through via its captured inlineImageRaw.Bytes
// instead, so this canonical (re-spaced) form never appears unless something
// was fixed.
func buildInlineImageBytes(params []PDFValue, data []byte) ([]byte, error) {
	var buf bytes.Buffer
	buf.WriteString("BI")
