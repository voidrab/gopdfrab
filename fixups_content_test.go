...
func TestContentLimitsFixerClearsViolations(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			Checks.Structure.HexStringOddLength,
		},
		{
...
func TestRewriteContentStreamDictDropsUndefinedOperator(t *testing.T) {
	...
	var ops []string
	newContentScanner(fixed.RawStream).scan(func(op string, _ []PDFValue) {
		ops = append(ops, op)
	})
	for _, op := range ops {
		if op == "XX" {
